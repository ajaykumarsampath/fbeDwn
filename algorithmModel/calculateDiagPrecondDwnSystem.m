...
if(dwnOptimModel.cell)
    for j = 1:Np
        ...
        dwnOptimModelPrecond.G{j,1} = diagDualHessian((j-1)*nz + 1:(j-1)*nz + nu,...
            (j-1)*nz + 1:(j-1)*nz + nu)*dwnOptimModelPrecond.G{j,1};
        dwnOptimModelPrecond.F{j,1} = diagDualHessian((j-1)*nz + nu + 1:j*nz,...
        ...
        dwnOptimModelPrecond.xmax((j-1)*nx + 1:j*nx, 1) = diagDualHessian((j-1)*nz + nu + 1:(j-1)*nz + nu...
