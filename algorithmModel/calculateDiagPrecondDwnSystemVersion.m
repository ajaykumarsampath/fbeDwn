function [ dwnOptimModelPrecond ] = calculateDiagPrecondDwnSystemVersion( dwnOptimModel, ...
    dualHessian, optionDiagPrecond)
%
...
if(dwnOptimModel.cell)
    ...
    for j = 1:Np
        ...
        dwnOptimModelPrecond.G{j,1} = diagDualHessian((j-1)*nz + 1:(j-1)*nz + rowMatG,...
            (j-1)*nz + 1:(j-1)*nz + rowMatG)*dwnOptimModelPrecond.G{j,1};
        dwnOptimModelPrecond.F{j,1} = diagDualHessian((j-1)*nz + rowMatG + 1:j*nz,...
            (j-1)*nz+ rowMatG+1:j*nz)*dwnOptimModelPrecond.F{j,1};
       ...
       % dwnOptimModelPrecond.equalEd((j-1)*rowMatEd + 1:j*rowMatEd, 1) = diagDualHessian((j-1)*nz...
        %    + nu + 1:(j-1)*nz + rowMatG, (j-1)*nz + 1:(j-1)*nz + nu)*dwnOptimModelPrecond.equalEd(...
         %   (j-1)*rowMatEd + 1:j*rowMatEd, 1);
        
        dwnOptimModelPrecond.xmax((j-1)*nx + 1:j*nx, 1) = diagDualHessian((j-1)*nz + rowMatG + 1:(j-1)*nz + rowMatG...
            + nx, (j-1)*nz + rowMatG + 1:(j-1)*nz + rowMatG + nx)*dwnOptimModelPrecond.xmax((j-1)*nx + 1:j*nx);
        dwnOptimModelPrecond.xmin((j-1)*nx + 1:j*nx, 1) = diagDualHessian((j-1)*nz + rowMatG + 1:(j-1)*nz + rowMatG...
            +nx, (j-1)*nz + rowMatG + 1:(j-1)*nz + rowMatG + nx)*dwnOptimModelPrecond.xmin((j-1)*nx + 1:j*nx);
        dwnOptimModelPrecond.xs((j-1)*nx + 1:j*nx, 1) = diagDualHessian((j-1)*nz + rowMatG + nx + 1:j*nz,...
            (j-1)*nz + rowMatG + nx + 1:j*nz)*dwnOptimModelPrecond.xmin((j-1)*nx + 1:j*nx);
    end
else
